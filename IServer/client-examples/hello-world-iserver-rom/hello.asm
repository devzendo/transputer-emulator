...
PAGE 60,132
	...
	start_time  EQU 0
; You can't do
;	ORG         (ResetCode - 8) - (CodeEnd - CodeStart)
; because ORG must be resolvable, and we don't know CodeStart or CodeEnd yet.
; So we have to know how long the code is, and set the ORG that
; many bytes back from the jumps that are just before MaxINT. Bit of a faff... 
; Examine the value of _XtraPadding to see how much padding is present between the end of the
; code and the end of the ROM.
...
; Adds final jumps at end of ROM:
	ORG		ResetCode - 8		; give space for longest possible jump encoding
JumpToCodeStart: 
	j		CodeStart
; The Transputer jumps here on reset, if booting from ROM - but there are only 2
; bytes for the jump. So a jump back a few bytes will fit in a 2 byte jump.
	ORG		ResetCode
	j		JumpToCodeStart
